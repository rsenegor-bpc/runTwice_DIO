...
typedef struct _bufferedAoData
{
   ...
   rtl_pthread_cond_t   event;       // condition used to signal the thread that an 
   rtl_pthread_mutex_t  event_lock;  // interrupt occured.
} tBufferedAoData;
...
int WaitBufferedAO(tBufferedAoData *pAoData, int timeoutms)
{
   ...
   retVal = rtl_pthread_mutex_lock(&pAoData->event_lock);
   if (retVal != 0)
   {
   ...
   rtl_timespec_add_ns(&timeout, timeoutms*1000000);

   retVal = rtl_pthread_cond_timedwait(&pAoData->event, &pAoData->event_lock, &timeout);
   if (retVal != 0)
   {
      if (retVal == ETIMEDOUT)
      {
   ...
   rtl_pthread_mutex_unlock(&pAoData->event_lock);
#else
...
unsigned int pd_isr(unsigned int trigger_irq,struct rtl_frame *frame) 
{
    ...
    { // interrupt pending
        ...
        if ((Events.AOutIntr & AOB_HalfDoneSC) || (Events.AOutIntr & AOB_HalfDoneSC)) 
        {
            ...
            rtl_pthread_cond_signal(&aoData.event);
        }
    }
...
int init_module(void)
{
   ...
   ret = rtl_pthread_cond_init(&aoData.event, NULL);
   if (ret != 0)
   {
   ...
   ret = rtl_pthread_mutex_init(&aoData.event_lock, NULL);
   if (ret != 0)
   {
...
void cleanup_module(void)
{
   ...
   rtl_pthread_cond_destroy(&aoData.event);
   rtl_pthread_mutex_destroy(&aoData.event_lock);
#endif
