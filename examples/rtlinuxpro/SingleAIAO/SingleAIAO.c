...
int main(int argc, char *argv[])
{
   ...
   mkfifo( "/dev/SingleAIAO", 0666);
   fifod = open("/dev/SingleAIAO", O_WRONLY | O_NONBLOCK);
   ...
   //pthread_attr_setcpu_np(&attrib, 1);  // Start this thread on CPU 1
   //pthread_attr_setreserve_np(&attrib, 1); // Reserve CPU1 to only run this thread                            
