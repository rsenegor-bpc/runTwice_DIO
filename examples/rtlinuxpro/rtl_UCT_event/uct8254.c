...
int InitUctGenSquareWave(tUctGenSquareWaveData *pUctData)
{
   ...
   if (retVal < 0)
   {
      printk("UctGenSquareWave: PdUctReset error %d\n", retVal);
      return retVal;;
   }
...
int StartUctGenSquareWave(tUctGenSquareWaveData *pUctData, double frequency)
{
   ...
   if (retVal < 0)
   {
      printk("UctGenSquareWave: _PdUctSetCfg failed with error code %d\n", retVal);
      return retVal;
   }
   ...
   retVal = pd_uct_write(pUctData->board, uctCtrl | (nbTicks << 8));
   if (retVal < 0)
   {
      printk("UctGenSquareWave: _PdUctWrite failed with error code %d\n", retVal);
      return retVal;
   }
   ...
   if(pUctData->gateSource == gateSoftware)
   {
      ...
      if (retVal < 0)
      {
         printk("UctGenSquareWave: _PdUctSwSetGate failed with error code %d\n", retVal);
         return retVal;
      }
   }
...
int StopUctGenSquareWave(tUctGenSquareWaveData *pUctData)
{
   ...
   if (retVal < 0)
   {
      printk("UctGenSquareWave: _PdUctRead failed with error code %d\n", retVal);
      return retVal;
   }

   //printk("UCT_GenSquareWave: counted %ld events\n", 65536-count);
...
int CleanUpUctGenSquareWave(tUctGenSquareWaveData *pUctData)
{
   ...
   if(pUctData->state == configured)
   {
      retVal = pd_uct_reset(pUctData->board);
      if (retVal < 0)
         printk("UctGenSquareWave: _PdUctReset error %d\n", retVal);
