...
int TestMeasPulseWidth(int handle, int timer);
...
int TestMeasPulseWidth(int handle, int timer)
{
   ...
   retVal = PdDspUctWaitForEvent(handle, timer, MeasPulseWidth, 10000);
   if(retVal < 0)
   {
   ...
   printf("TestCountEvent: %lu events were counted, width is %f ms\n", count, count * 1000.0 / 33000000.0);
