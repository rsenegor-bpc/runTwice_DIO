...
int TestCountEvent(int handle, int timer);
...
int TestCountEvent(int handle, int timer)
{
   ...
   retVal = PdDspUctConfig(handle, 
                           ...
                           CountEvent,
                           1,                   //divider
                           20,                //stop counting after 2000 events
                           0,                //ticks2
                           0);                  //inverted
   if(retVal < 0)
   {
   ...
   /*retVal = PdDspUctWaitForEvent(handle, timer, CountEvent, 10000);
   if(retVal < 0)
   {
   ...
   while(!bStop)
   {
      ...
      printf("TestCountEvent: %lu events were counted\n", count);
