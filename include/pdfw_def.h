...
/* High-speed boards (with 25MHz oscillator)*/
#define PD_IS_HS(id)    ((id==0x161)||(id==0x261)||(id==0x18A)||(id==0x154)||(id==0x155))
/*                       PD2-DIO-64TS PDXI-DIO-64TS PDL-DIO-64TS PD2-AO-96/16HS PD2-AO-8/16HSG*/
...
#define RTModeBmPageXFers      1/*4*/    /* # Blocks per page*/
...
/*???????????????????????????????????????????????????????*/
/* ??TransferSample - Number of the samples which are necessary for transferring on host*/
/* ??For example: necessary transfer on host 712 samples from FIFO*/
/* ??(Cahannel list size = 8 (BurstSize), # Burst = 89 (Burst) 8*89=712 Samples)*/
#define RTModeTransferSample   712
/* ??Programming DMA for fransferring RTModeTransferSample*/
/* ??= ((dwRTModeAIBM_DEFDMASIZE+1)*dwRTModeAIBM_DEFBURSTS) + (dwRTModeAIBM_DEFDMASIZELAST+1) =*/
/* ??= (15+1)*44 + (7+1) = 704 + 8 = 712 (samples)*/
/* This parameters calculated in PdAInAsyncInit*/
/*????????????????????????????????????????????????????????*/
...
/* IVEC table is full and DIO firmware ignores PD_UCT_XXXX commands (because DIO */
/* board have DSPUCT subsystem, instead of UCT). We can use PD_UCTWRITE and PD_UCTREAD */
/* addresses for new commands - PD_DI0256RD_ALL and PD_DI0256WR_ALL */
#define PD_DIO256RD_ALL    PD_UCTREAD        /* DIO 256 Command (Read  4 DIO ports)*/
#define PD_DIO256WR_ALL    PD_UCTWRITE       /* DIO 256 Command (Write 4 DIO ports)*/
...
#define AIBM_TXSIZE1024 1024    /* 4K/8K/16K/32K/64K FIFO internal BM buffer size, samples*/
#define AIBM_TXSIZE512  512     /* 1K/2K FIFO internal BM buffer size, samples*/
...
#define AOB_TSEQTMR0EN  (1L<<11) /* Use external signal to restart a Time sequence*/
                                 /* The external signal should be connected to TMR0*/
                                 /* (Interruption from TMR0 restart the sequence)*/
#define AOB_TSEQTMR0EDGE (1L<<12)/* External signal edge (rising=0, falling=1)*/
#define AOB_REGENERATE  (1L<<13) /* Switch to regenerate mode*/
#define AOB_REGENERATE_SINGLESHOT (1L<<14) /* Outputs a single scan with output rate up to 8.25MHz (if AOB_DMAEN set)*/
                                 /* This flag is only valid if AOB_REGENERATE is set*/
...
#define AOB_AOUT32      (1L<<30) /* switch to PD2-AO board (format: (channel<<16)|(value & 0xFFFF))*/
#define AOB_DOUT32      (1L<<30) /* switch to PD2-DIO board (format: (channel<<16)|(value & 0xFFFF))*/
#define AOB_DSPWF       (1L<<30) /* DSP WF mode - waveform is calculated by DSP on the fly*/
