...
// DESCRIPTION:
//
//          PowerDAQ Linux driver interface to event-related FW functions
//
...
int pd_enable_events(int board, tEvents* pEvents)
{
    ...
    if ( pEvents->ADUIntr )
    {
        ...
        DPRINTK_T("pd_enable_events ADUIntr: 0x%x\n", dwIntrStatus);
    ...
    if ( pEvents->AIOIntr )
    {
        ...
        // Set new interrupt mask bits and clear specified interrupt status bits.
        DPRINTK_T("pd_enable_events AIOIntr: 0x%x\n", dwIntrStatus);
    ...
    if ( pEvents->AInIntr )
    {
        ...
        // Set new interrupt mask bits and clear specified interrupt status bits.
        DPRINTK_T("pd_enable_events AInIntr: 0x%x\n", dwIntrStatus);
    ...
    if ( pEvents->AOutIntr )
    {
        ...
        // Set new interrupt mask bits and clear specified interrupt status bits.
        DPRINTK_T("pd_enable_events AOutIntr: 0x%x\n", dwIntrStatus);
...
int pd_disable_events(int board, tEvents* pEvents)
{
    ...
    if ( pEvents->ADUIntr )
    {
        ...
        // Set new interrupt mask bits and clear specified interrupt status bits.
        DPRINTK_T("pd_disable_events ADUIntr: 0x%x\n", pd_board[board].FwEventsConfig.ADUIntr);
    ...
    if ( pEvents->AIOIntr )
    {
        ...
        // Set new interrupt mask bits and clear specified interrupt status bits.
        DPRINTK_T("pd_disable_events AIOIntr: 0x%x\n", pd_board[board].FwEventsConfig.AIOIntr);
    ...
    if ( pEvents->AInIntr )
    {
        // Clear specified mask bits.
        pd_board[board].FwEventsConfig.AInIntr &=
            ~(pEvents->AInIntr & (  AIB_StartIm | AIB_StopIm | AIB_SampleIm | AIB_ScanDoneIm
                                  | AIB_ErrIm | AIB_BMPgDoneIm | AIB_BMErrIm ));

        // Set new interrupt mask bits and clear specified interrupt status bits.
        DPRINTK_T("pd_disable_events AInIntr: 0x%x\n", pd_board[board].FwEventsConfig.AInIntr);
    ...
    if ( pEvents->AOutIntr )
    {
        ...
        // Set new interrupt mask bits and clear specified interrupt status bits.
        DPRINTK_T("pd_disable_events AOutIntr: 0x%x\n", pd_board[board].FwEventsConfig.AOutIntr);
                
        Status = pd_aout_set_events(board, pd_board[board].FwEventsConfig.AOutIntr);
    }
...
int pd_set_user_events(int board, u32 subsystem, u32 events)
{
    ...
    DPRINTK_P("pd_set_user_events subsystem=%d, events=0x%08X\n", subsystem, events);
    ...
    if ( subsystem == AnalogIn )
    {
                                   ...
                                   | eConvError | eScanError | eDataError | eBufferError | eFrameDone | eBufferDone | eBufferWrapped) )
        {
            ...
            if ( events & eScanDone )
                FwEvents.AInIntr |= AIB_ScanDoneIm | AIB_ScanDoneSC;
	        ...
	        DPRINTK_T("pd_enable_events returns %d\n", Status);
        }
    } 
    ...
    {
       DPRINTK_E("pd_set_user_event Din : event = %d\n", events); 
       
       // Set driver event notification bits and clear event status bits.
       pd_board[board].DinSS.dwEventsNotify |= events;
       pd_board[board].DinSS.dwEventsStatus &= ~events;
    ...
    else if (subsystem == CounterTimer || subsystem == DSPCounter)
    {
        // Set driver event notification bits and clear event status bits.
    ...
    {
    	DPRINTK_F("error in pd_set_user_events - unknown subsystem\n");
    	Status = 0;
    }
    return Status;
}
...
int pd_get_user_events(int board, u32 subsystem, u32* events)
{
    ...
    {
        // Set driver events.
        EventsStatus = pd_board[board].DinSS.dwEventsStatus;
...
int pd_immediate_update(int board)
{
    ...
    pd_board[board].bImmUpdate = FALSE;
...
void pd_debug_show_events ( tEvents *Event, char* msg)
{
    ...
    // ADUIntr -----------------------------
    DPRINTK_T("--- ADUIntr --> 0x%x", Event->ADUIntr);
    str[0] = '\0';
    ...
    // AIOIntr -----------------------------
    DPRINTK_T("--- AIOIntr --> 0x%x", Event->AIOIntr);
    str[0] = '\0';
...
//    if (Event->AIOIntr & AIB_FNEIm)		strcat( str, "AIB_FNEIm ");
    ...
    // AInIntr -----------------------------
    DPRINTK_T("--- AInIntr --> 0x%x", Event->AInIntr);
    str[0] = '\0';
    ...
    // AOutIntr -----------------------------
    DPRINTK_T("--- AOutIntr --> 0x%x", Event->AOutIntr);
    str[0] = '\0';
