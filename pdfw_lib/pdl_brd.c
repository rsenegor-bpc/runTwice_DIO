...
// DESCRIPTION:
//
//          PowerDAQ Linux driver interface to board-level FW functions
//
...
{
   DPRINTK_P("pd_adapter_enable_interrupt: set to %d\n", val);
...
int pd_adapter_get_board_status(int board, tEvents* pEvent)
{
   ...
   DPRINTK_P("pd_adapter_get_board_statusi: ");

   // read data block
   res = pd_dsp_read(board); 
   DPRINTK_P("Board 0x%x ", res);
   pEvent->Board = res;

   res = pd_dsp_read(board); 
   DPRINTK_P("ADUIntr 0x%x ", res);
   pEvent->ADUIntr = res;

   res = pd_dsp_read(board); 
   DPRINTK_P("AIOIntr 0x%x ", res);
   pEvent->AIOIntr = res;

   res = pd_dsp_read(board); 
   DPRINTK_P("AInIntr 0x%x ", res);
   pEvent->AInIntr = res;

   res = pd_dsp_read(board); 
   DPRINTK_P("AOutIntr 0x%x\n", res);
   pEvent->AOutIntr = res;
...
int pd_adapter_eeprom_read(int board, u32 dwMaxSize, uint16_t *pwReadBuf)
{
    ...
    if ( dwEepromSize == 0  || dwEepromSize > PD_EEPROM_SIZE)
        return -1;
